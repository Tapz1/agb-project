...
def upload_testimonial_images(image_form, existing_photos):
    path_slice = current_app.app_context().app.config['PATH_SLICE']
    testimonial_upload_path = current_app.app_context().app.config['PROJECT_UPLOAD_FOLDER']
    ...
    try:
        ...
        if uploaded_images:
            ...
            date = request.form['project_date']
            try:
                pathlib.Path(testimonial_upload_path, project_name).mkdir(
                    exist_ok=False)
            except Exception as e:
                print(e)
                flash("There's already an existing project with that name", 'danger')
            ...
            try:
                project_path = os.path.join(testimonial_upload_path, project_name)
